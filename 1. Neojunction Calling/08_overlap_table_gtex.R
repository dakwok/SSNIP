# Title: "Step 8: SJ Overlap Table GTEx"
...
# 2. Select the 9166 files
# GTEx Meta Data:
setwd(directory_meta)
meta_gtex = read_tsv("07102020_gtex_id_annotation_9166.tsv", col_names = T)

# 3. Bind the ordered list of junctions to the sj.out.tab files for GTEx
setwd(directory_samples)
#list_rnaseq.files = list.files()[grepl("SJ.out.tab",list.files())] #15,966 total files

matching_files <- list.files(directory_samples, recursive = TRUE, full.names = TRUE) %>%
  keep(str_detect(., regex(paste(meta_gtex$sample.id, collapse = "|"), ignore_case = TRUE)))

# Filter strings for ones that contain "SJ.out.tab"
list_rnaseq.files <- grep("SJ.out.tab", matching_files, value = TRUE)
...
setwd(directory_out)
filename_output.overlap.table = "2023_0623_gtex_sj_overlap_table.tsv"
write_tsv(dataframe_overlap.table, filename_output.overlap.table, na = "NA", col_names = T, quote_escape = "double") ;


filename_output.retain = "2023_0623_gtex_sj_count_table_sj.out.tab_retained.tsv"
